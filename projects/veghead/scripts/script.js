...
function getPlaces(loc) {
  ...
  geocoder.geocode({ address: address }, function(results, status) {
    if (status == google.maps.GeocoderStatus.OK) {
      ...
      const errMsg = `<p>Please enter a valid location.</p>`;
...
function createMarker(obj) {
  let contentString = `<strong>${
    obj.name
  }</strong> | <a href="https://www.google.com/maps/dir/?api=1&destination=${
    obj.name
  }&destination_place_id=${obj.place_id}" target="_blank">${obj.vicinity}</a>`;
